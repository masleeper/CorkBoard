        ...
        public List<AlertData> getAlerts(string state, string county)
        {
            ...
            try
            {
                    ...
                    wJson["features"][0] != null)
                {
                    ...
                    {
                        AlertData data1;
                        if (((string)wJson["features"][i]["properties"]["areaDesc"]).Contains(county))
                        {
                            data1.msg = wJson["features"][i]["properties"]["event"] != null ? (string)wJson["features"][i]["properties"]["event"] : "";
