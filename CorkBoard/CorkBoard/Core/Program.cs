...
namespace CorkBoardCoreTest
{
    class Core
    {
        static string wxurl; //Full URL for weather API in our zone. In the future, maybe replace with zip code and structure an appropriate API link on our own?
        static string imgurl; //Full URL for image to display.
        static int wxrefresh = 300; //Time in seconds to refresh weather - Lower for quicker refresh. BEWARE RATE LIMITING.
        static int imgrefresh = 120; //Time in seconds to refresh image - Lower means quicker refresh.
        static int clkrefresh = 5; //Time in seconds to check clock - Lower means higher accuracy but potentially more problems. Not sure this should be a user-defined global, but adding it in for now.
        static int[] btheme = new int[12]; //Contains a basic color scheme, four RGB values. Background1, Text1, Background2, Color2.


        static bool GetInitializers() //Reads CorkBoard.ini and populates the globals with accurate values.
        //returns true if initializers were read okay, false if any issue arose.
        {
            Console.WriteLine("Begin reading .INI values.");
            string[] inilines = File.ReadAllLines("CorkBoard.ini");
